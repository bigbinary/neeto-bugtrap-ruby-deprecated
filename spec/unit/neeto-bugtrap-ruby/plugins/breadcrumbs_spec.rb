  ...
  describe NeetoBugtrap::Plugins::RailsBreadcrumbs do
    ...
    describe '.send_breadcrumb_notification' do
      it 'adds a breadcrumb with overridden config settings' do
        ...
        expect(NeetoBugtrap).to receive(:add_breadcrumb).with('config message',
                                                              { category: :test,
                                                                metadata: data.merge({ duration: 99 }) })
        described_class.send_breadcrumb_notification('message', 99, config, data)
      end

      it 'adds a breadcrumb with defaults' do
        expect(NeetoBugtrap).to receive(:add_breadcrumb).with('message',
                                                              { category: :custom, metadata: { duration: 100 } })
      ...
      describe ':select_keys', skip: !Hash.method_defined?(:slice) do
        it 'can filter metadata' do
          ...
          expect(NeetoBugtrap).to receive(:add_breadcrumb).with(anything,
                                                                hash_including(metadata: hash_not_including(removed_data)))
