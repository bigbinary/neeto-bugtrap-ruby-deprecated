...
describe NeetoBugtrap::Notice do
  ...
  def assert_accepts_exception_attribute(attribute, args = {}, &block)
    ...
    block ||= -> { exception.send(attribute) }
  ...
  it 'does not filter the backtrace' do
    notice = build_notice(config: build_config('request.filter_keys': ['number']),
                          backtrace: ['foo:1:in `bar\''])
    json = notice.to_json
    payload = JSON.parse(json)
  ...
  %w[params session cgi_data].each do |var|
    ...
    context "when #{var} is excluded" do
      it 'sends default value' do
        ...
        notice = build_notice(:config => build_config("request.disable_#{cfg}": true),
                              var.to_sym => { var => 'hello' })
        json = notice.to_json
        payload = JSON.parse(json)
  ...
  describe '#ignore?' do
    it 'does not ignore an exception not matching ignore filters' do
      ...
      config.exception_filter { |_n| false }
  ...
  describe '#context' do
    ...
    it "doesn't mutate global context" do
      ...
      expect { build_notice(global_context: global_context, context: { 'foo' => 'bar' }) }.not_to change {
                                                                                                    Thread.current[:__neetobugtrap_context]
                                                                                                  }
    end
    ...
    context 'with a rack environment hash' do
      ...
      if defined?(::Rack) && Gem::Version.new(Rack.release) < Gem::Version.new('1.3')
        it 'parses params which are malformed in Rack >= 1.3' do
          env = Rack::MockRequest.env_for('http://www.example.com/explode', method: 'POST',
  ...
  describe '#as_json' do
    ...
    it 'trims error message to 64k' do
      ...
      expect(65_536...65_556).to cover notice.as_json[:error][:message].bytesize
    end
  ...
  context 'custom fingerprint' do
    ...
    context 'fingerprint is a callback which accesses notice' do
      it 'can access request information' do
        notice = build_notice({ params: { key: 'foo' }, fingerprint: ->(n) { n.params[:key] } })
        expect(notice.fingerprint).to eq 'foo'
        expect(notice.as_json[:error][:fingerprint]).to eq '0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33'
      end
    end
  ...
  context 'with a backtrace' do
    ...
    it 'passes its backtrace filters for parsing' do
      ...
      expect(NeetoBugtrap::Backtrace).to receive(:parse).with(@backtrace_array,
                                                              hash_including(filters: array_including('foo'))).and_return(double(to_a: []))
    ...
    it 'passes backtrace line filters for parsing' do
      ...
      @backtrace_array.each do |line|
        expect(NeetoBugtrap::Backtrace::Line).to receive(:parse).with(line,
                                                                      hash_including({ filters: array_including('foo'),
                                                                                       config: config }))
      end
  ...
  context 'adding tags' do
    ...
    it 'allows non-word characters in tags while stripping whitespace' do
      expect(build_notice(tags: 'word,  with_underscore ,with space, with-dash,with$special*char').tags).to eq(%w[word
                                                                                                                  with_underscore with space with-dash with$special*char])
    end
  end
  ...
  context 'exception cause' do
      ...
      context "when raising #{error_class} with a cause" do
        ...
        context 'and the :cause option is also present' do
          it 'prefers the option' do
            exception = error_class.new('bugtraps!')
            exception.cause = build_cause
            causes = build_notice(exception: exception,
                                  cause: build_cause(message: 'this cause was passed explicitly')).as_json[:error][:causes]
        ...
        context 'and there is a current exception' do
          it "prefers the notice's exception's cause" do
            ...
            begin
              raise StandardError, 'this should not be the cause'
            rescue StandardError
              causes = build_notice(exception: exception).as_json[:error][:causes]
            end
    ...
    context 'when there is a current global exception' do
      it 'uses the global cause' do
        begin
          raise StandardError, 'expected current cause'
        rescue StandardError
    ...
    context 'when the :cause option is present' do
      it 'uses the cause option' do
        begin
          raise StandardError, 'unexpected current cause'
        rescue StandardError
