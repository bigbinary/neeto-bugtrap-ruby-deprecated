...
describe NeetoBugtrap::Util::Stats do
  describe '.memory' do
    subject { NeetoBugtrap::Util::Stats.memory }
    ...
    describe '.keys' do
      subject { NeetoBugtrap::Util::Stats.memory.keys }
    ...
    context 'when mathn is required' do
      ...
      it 'converts Rational to Float', if: RUBY_VERSION < '2.5.0' do
        expect(NeetoBugtrap::Util::Stats.memory[:total]).to be_a Float
        expect(NeetoBugtrap::Util::Stats.memory[:free]).to be_a Float
        expect(NeetoBugtrap::Util::Stats.memory[:buffers]).to be_a Float
        expect(NeetoBugtrap::Util::Stats.memory[:cached]).to be_a Float
      end
    end
  end

  describe '.load' do
    subject { NeetoBugtrap::Util::Stats.load }

    before do
      stub_const('NeetoBugtrap::Util::Stats::HAS_LOAD', true)
      allow(IO).to receive(:read).with('/proc/loadavg').and_return('22.58 19.66 15.96 20/2019 2')
    end

    describe '.keys' do
      subject { NeetoBugtrap::Util::Stats.load.keys }
