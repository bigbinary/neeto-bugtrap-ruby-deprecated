...
describe NeetoBugtrap::Util::Sanitizer do
  ...
  describe '#sanitize' do
    ...
    let(:expected_hash) do
      {}.tap do |h|
        max_depth.times.each do |i|
          h = h[i.to_s] = (i < max_depth - 1 ? { string: 'string' } : '[DEPTH]')
        end
      end
    end
    ...
    context 'with filters' do
      ...
      let!(:filters) do
        ['abc', :def, /private/, /^foo_.*$/, 'nested.string', /nested\.regexp$/, lambda { |k, v|
                                                                                   if k == 'block'
                                                                                     v.replace('block filter')
                                                                                   end
                                                                                 }]
      end
