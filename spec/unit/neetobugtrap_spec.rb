...
describe NeetoBugtrap do
  ...
  describe '#notify' do
    ...
    it 'creates and send a notice for an exception' do
      ...
      expect(NeetoBugtrap::Notice).to receive(:new).with(config,
                                                         hash_including(exception: exception)).and_return(notice)
    ...
    it 'creates and send a notice for a hash' do
      ...
      expect(NeetoBugtrap::Notice).to receive(:new).with(config,
                                                         hash_including(error_message: 'uh oh')).and_return(notice)
      expect(worker).to receive(:push).with(notice)
    ...
    it 'creates and sends a notice for an exception and hash' do
      ...
      expect(NeetoBugtrap::Notice).to receive(:new).with(config,
                                                         hash_including(notice_args.merge(exception: exception))).and_return(notice)
      expect(worker).to receive(:push).with(notice)
    ...
    it 'sends a notice with a string' do
      ...
      expect(NeetoBugtrap::Notice).to receive(:new).with(config,
                                                         hash_including(error_message: 'the test message')).and_return(notice)
      expect(worker).to receive(:push).with(notice)
  ...
  describe '#configure' do
    ...
    it 'configures the singleton' do
      ...
      end.to change { NeetoBugtrap.config.get(:api_key) }.from(nil).to('test api key')
    end
