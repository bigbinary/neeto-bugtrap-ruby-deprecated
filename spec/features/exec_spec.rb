...
feature "Running the exec cli command" do
  ...
  it "quietly executes the requested command" do
    output = capture(:stdout) { NeetoBugtrapRuby::CLI.start(%w[exec --api-key=test-api-key ls]) }
  ...
  context "when the options are invalid" do
    it "notifies the user" do
      output = capture(:stdout) do
        expect{ NeetoBugtrapRuby::CLI.start(%w[exec --api-key= ls]) }.to raise_error(SystemExit)
      end
  ...
  context "when the command fails due to a non-zero exit code" do
    it "notifies NeetoBugtrap of the failure" do
      output = capture(:stdout) do
        expect{NeetoBugtrapRuby::CLI.start(%w[exec --api-key=test-api-key this-command-should-not-exist])}.to raise_error(SystemExit)
      end
      expect(output).to match(/failed.+this-command-should-not-exist/im)
      expect(output).to match(/Successfully notified NeetoBugtrap/i)
    end
  end
...
context "when Rails is not detected due to a missing environment.rb" do
    it "skips rails initialization without logging" do
      output = capture(:stdout) { NeetoBugtrapRuby::CLI.start(%w[exec --api-key=test-api-key --skip-rails-load ls]) }
  ...
  context "when Rails is detected via the presence of environment.rb" do
    ...
    it "does not skip rails initialization when false or not set" do
      output = capture(:stdout) { NeetoBugtrapRuby::CLI.start(%w[exec --api-key=test-api-key --skip-rails-load=false ls]) }
      expect(output).to_not match(/Skipping Rails initialization/i)
