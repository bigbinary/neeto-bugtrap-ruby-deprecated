feature "Running the notify CLI command" do
  ...
  it "requires the --message flag" do
    output = capture(:stderr) { NeetoBugtrap::CLI.start(%w[notify]) }
  ...
  context "with a message" do
    it "reports an exception with a default class" do
      expect(run_command('neetobugtrap notify --message "Test error message"')).to be_successfully_executed
      assert_notification('error' => {'class' => 'CLI Notification', 'message' => 'Test error message'})
    end

    it "overrides the class via --class flag" do
      expect(run_command('neetobugtrap notify --class "MyClass" --message "Test error message"')).to be_successfully_executed
    ...
    it "overrides the API key via --api-key flag" do
      expect(run_command('neetobugtrap notify --message "Test error message" --api-key my-key')).to be_successfully_executed
  ...
  context "when Rails is not detected due to a missing environment.rb" do
    it "skips rails initialization without logging" do
      expect(run_command('neetobugtrap notify --message "Test error message" --skip-rails-load')).to be_successfully_executed
  ...
  context "when Rails is detected via the presence of environment.rb" do
    ...
    it "does not skip rails initialization when false or not set" do
      expect(run_command('neetobugtrap notify --message "Test error message" --skip-rails-load=false')).to be_successfully_executed
      expect(all_output).to_not match(/Skipping Rails initialization/i)
