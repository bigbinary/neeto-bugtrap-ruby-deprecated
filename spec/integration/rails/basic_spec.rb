require_relative '../rails_helper'

describe "Rails integration", if: RAILS_PRESENT, type: :request do
  load_rails_hooks(self)

  it "inserts the middleware" do
    expect(RailsApp.middleware).to include(NeetoBugtrapRuby::Rack::ErrorNotifier)
  end

  it "reports exceptions" do
    NeetoBugtrapRuby.flush do
      get "/runtime_error"
      expect(response.status).to eq(500)
    end

    expect(NeetoBugtrapRuby::Backend::Test.notifications[:notices].size).to eq(1)
  end

  it "sets the root from the Rails root" do
    expect(NeetoBugtrapRuby.config.get(:root)).to eq(Rails.root.to_s)
  end

  it "sets the env from the Rails env" do
    expect(NeetoBugtrapRuby.config.get(:env)).to eq(Rails.env)
  end

  context "default ignored exceptions" do
    it "doesn't report exception" do
      NeetoBugtrapRuby.flush { get "/record_not_found" }

      expect(NeetoBugtrapRuby::Backend::Test.notifications[:notices]).to be_empty
    end
  end
end
